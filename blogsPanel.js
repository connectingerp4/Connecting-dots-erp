// server.js - Full Backend Code with Keywords Integration
...
// CORRECTED BLOG SCHEMA WITH COURSES SUPPORT AND KEYWORDS
const blogSchema = new mongoose.Schema({
  title: { type: String, required: true },
  slug: { type: String, required: true, unique: true, lowercase: true, trim: true, index: true },
  content: { type: String, required: true },
  category: { type: String, required: true },
  subcategory: { type: String, required: true, enum: ['Article', 'Tutorial', 'Interview Questions'] },
  ...
  status: { type: String, enum: ['Trending', 'Featured', "Editor's Pick", 'Recommended', 'None'], default: 'None' },
  // Tags Field
  tags: {
    type: [String],
    default: [],
  ...
  // NEW: Keywords Field (similar to tags, for SEO/search)
  keywords: {
    type: [String],
    default: [],
    validate: {
...
blogSchema.index({ keywords: 1 }); // NEW: Index for keywords
...
const getPublicIdFromUrl = (url) => {
  ...
  try {
    ...
    if (versionIndex !== -1 && urlParts.length > versionIndex + 2) {
      ...
      const publicIdWithFolder = urlParts.slice(urlParts.lastIndexOf('upload') + 2).join('/');
...
const deleteCloudinaryImage = async (publicId) => {
  if (!publicId || typeof publicId !== 'string' || !publicId.trim()) {
    console.warn('Invalid publicId for deletion:', publicId);
    return;
  }
  try {
...
const deleteAllBlogImages = async (blog) => {
  ...
  if (blog.imagePublicId) deletePromises.push(deleteCloudinaryImage(blog.imagePublicId));
  // Delete banner image
  if (blog.bannerImagePublicId) deletePromises.push(deleteCloudinaryImage(blog.bannerImagePublicId));
  ...
  if (blog.courseImagesData && Array.isArray(blog.courseImagesData) && blog.courseImagesData.length > 0) {
    blog.courseImagesData.forEach(imageData => {
      if (imageData && imageData.publicId) deletePromises.push(deleteCloudinaryImage(imageData.publicId));
    });
  }
...
// NEW: Helper function to process keywords (similar to tags)
const processKeywords = (keywordsInput) => {
  if (!keywordsInput) return [];
  let keywords;
  try {
    if (typeof keywordsInput === 'string') {
      keywords = JSON.parse(keywordsInput);
    } else if (Array.isArray(keywordsInput)) {
      keywords = keywordsInput;
    }
  } catch (e) {
    if (typeof keywordsInput === 'string') {
      keywords = keywordsInput.split(',').map(kw => kw.trim()).filter(Boolean);
    } else {
      keywords = [];
    }
  }
...
async function cleanupCourseImages(imagesData) {
  const cleanup = imagesData.map(img => img?.publicId ? deleteCloudinaryImage(img.publicId) : Promise.resolve());
...
async function processCourses(coursesInput, files) {
  ...
  for (let i = 0; i < parsed.length; i++) {
    ...
    if (!course || typeof course !== 'object' || !course.name?.trim() || !course.description?.trim() || !course.courseUrl?.trim()) {
    ...
    const { path: courseImagePath, publicId: courseImagePublicId } = await processCourseImage(
      course,
      courseImages[i],
      courseImagesData,
      i
    ...
    validCourses.push({
      ...
      courseImage: courseImagePath || null,
      courseImagePublicId: courseImagePublicId || null,
    });
  }
  return { validCourses, courseImagesData, error: null };
}
...
const requireRole = (roles) => {
  return (req, res, next) => {
    if (!req.user) return res.status(401).json({ message: 'Access Denied: Authentication required' });
...
app.post('/api/auth/register', async (req, res) => {
  try {
    const { username, email, password, role } = req.body; // user: req.body;
    if (!username || !password) return res.status(400).json({ message: 'Username and password are required' });
    const existingUser = await User.findOne({ $or: [{ username }, email ? { email } : {}] });
    if (existingUser) return res.status(409).json({ message: 'User with that username or email already exists' });
    const user = new User({ username, email, password, role });
    await user.save();
    res.status(201).json({ message: 'User registered successfully!', user: user.profile });
  } catch (err) {
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    res.json({ message: 'Logged in successfully', token, user: user.profile });
  } catch (err) {
...
app.put('/api/auth/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (userToUpdate.username === 'admin' && username && username !== 'admin') {
      return res.status(403).json({ message: 'Cannot change main admin username' });
    }
    if (role === 'superadmin' && req.user.role.toLowerCase() !== 'superadmin') {
      return res.status(403).json({ message: 'Only superadmin can promote users to superadmin role' });
    }
    const updateData = {};
...
app.get('/api/blogs', async (req, res) => {
  try {
    const { category, subcategory, status, tags, keywords, limit, skip } = req.query; // NEW: Added keywords to query
    let query = {};
    ...
    // NEW: Add keyword filtering support
    if (keywords) {
...
app.get('/api/blogs/keywords', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching keywords', error: err.message });
  }
});
...
app.get('/api/blogs/search/tags', async (req, res) => {
  try {
    ...
    if (!tags) return res.status(400).json({ message: 'Tags parameter is required' });
    ...
    res.json({ blogs, total, limit: parseInt(limit) || 10, skip: parseInt(skip) || 0 });
  } catch (err) {
...
// NEW: Search blogs by keywords
app.get('/api/blogs/search/keywords', async (req, res) => {
  try {
    const { keywords, limit, skip } = req.query;
    if (!keywords) return res.status(400).json({ message: 'Keywords parameter is required' });
    const keywordArray = Array.isArray(keywords) ? keywords : keywords.split(',').map(kw => kw.trim());
    const [blogs, total] = await Promise.all([
      Blog.find({ keywords: { $in: keywordArray } })
        .sort({ createdAt: -1 })
        .limit(parseInt(limit) || 10)
        .skip(parseInt(skip) || 0)
        .lean(),
      Blog.countDocuments({ keywords: { $in: keywordArray } })
    ]);
    res.json({ blogs, total, limit: parseInt(limit) || 10, skip: parseInt(skip) || 0 });
  } catch (err) {
    console.error('Error searching blogs by keywords:', err);
    res.status(500).json({ message: 'Error searching blogs', error: err.message });
  }
});
...
app.get('/api/blogs/:id', async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({ message: 'Invalid Blog ID format' });
...
app.post('/api/blogs', authenticateToken, uploadFields, async (req, res) => {
  try {
      ...
      try {
        ...
        processedCourses = coursesArray.map((course, index) => {
          ...
          if (courseImage) {
            ...
            courseImagePublicId = courseImage.filename || getPublicIdFromUrl(courseImage.path);
    ...
    const processedKeywords = processKeywords(keywordsInput);
    const newBlog = new Blog({
      ...
      status: status || 'None',
      tags: processedTags,
      keywords: processedKeywords, // NEW: Added processedKeywords
    ...
    if (req.files) {
      const filesToClean = [];
      if (req.files.image && req.files.image[0]) filesToClean.push(req.files.image[0].filename || getPublicIdFromUrl(req.files.image[0].path));
      if (req.files.bannerImage && req.files.bannerImage[0]) filesToClean.push(req.files.bannerImage[0].filename || getPublicIdFromUrl(req.files.bannerImage[0].path));
      if (req.files.courseImages) req.files.courseImages.forEach(file => filesToClean.push(file.filename || getPublicIdFromUrl(file.path)));
      // Delete all uploaded files
      filesToClean.forEach(async (publicId) => await deleteCloudinaryImage(publicId));
    }
...
// Update a blog WITH COURSES SUPPORT - Improved Implementation with Enhanced Logging and Validation
app.put('/api/blogs/:id', authenticateToken, uploadFields, async (req, res) => {
  try {
    ...
    // NEW: Process keywords if provided
    if (req.body.keywords !== undefined) {
      try {
        updatedData.keywords = processKeywords(req.body.keywords);
      } catch (e) {
    ...
    if (err?.code === 11000 && err?.keyPattern?.slug) {
      return res.status(409).json({ message: 'A blog with a similar title/slug already exists', error: err.message });
    }
    if (err?.name === 'ValidationError') {
      const errors = Object.values(err.errors).map(e => e.message);
      return res.status(400).json({ message: 'Validation failed', errors, error: err.message });
    }
